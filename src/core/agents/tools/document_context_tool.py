...
async def _document_context_implementation(
    ...
    """
    try:
        # Use default client if not specified (single Instagram account per app)
        if not client_id:
            client_id = "default_client"
        
        logger.info(f"Document context tool called for client_id: '{client_id}'")
        ...
        async with session_factory() as session:
            ...
            context = await document_context_service.get_client_context(
                client_id=client_id,
                session=session
            )
            ...
            formatted_output = f"✅ Business Documents Context for client '{client_id}':\n\n{context}\n\n"
            formatted_output += (

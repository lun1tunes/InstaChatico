...
def upgrade() -> None:
    ...
    result = connection.execute(
        sa.text("SELECT 1 FROM pg_type WHERE typname = 'answerstatus'")
    ).fetchone()
    ...
    op.create_table('question_messages_answers',
    ...
    op.add_column('question_messages_answers', 
                  sa.Column('processing_status', postgresql.ENUM('PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', 'RETRY', name='answerstatus'), nullable=False, server_default='PENDING'))
    
    op.create_index(op.f('ix_question_messages_answers_comment_id'), 'question_messages_answers', ['comment_id'], unique=True)
